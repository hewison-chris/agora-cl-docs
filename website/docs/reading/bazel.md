...
This page explains why Agora-cl uses a special build system called [Bazel](https://bazel.build) to compile everything in our monorepository.
...
Thankfully, Agora-cl also builds with `go` and `go` modules as well, so your code editor can use its regular Go support when developing for Agora-cl.
...
All dependencies in the Agora-cl monorepo live in a file called `deps.bzl` at the top-level of the repository [here](https://github.com/zeroone-boa/agora-cl/blob/develop/deps.bzl)

### How to add new dependencies

Adding new dependencies to Agora-cl requires specific changes. We have prepared a comprehensive [DEPENDENCIES.md](https://github.com/zeroone-boa/agora-cl/blob/master/DEPENDENCIES.md) explaining the required process.
...
To read comprehensive instructions on how to build Agora-cl's docker images for your own use, see [here](/docs/install/install-with-bazel).
...
Building Agora-cl with Go is possible, but it will use precompiled cryptography to build the final executable. Additionally, it will not perform the compile-time optimizations Bazel does, and can have unexpected issues as you are relinquishing reproducible, hermetic builds which Bazel provides. We always recommend Bazel as the only way to run Agora-cl if you are planning on running it.
