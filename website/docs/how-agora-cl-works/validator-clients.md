...
Although [beacon nodes](./agora-cl node) handle network synchronisation, drawing consensus and performing several other low-level functions, the role of [validators](/docs/terminology#validator) whom stake BOA to in order to perform block [proposals](/docs/terminology#propose) and [attestations](/docs/terminology#attest) are an equally critical component of the Ethereum Agora chain.
...
A validator instance is permitted to begin participating in the network once 40,000 BOA is locked up in a [validator deposit contract](./validator-deposit-contract). Validators are tasked with correctly [proposing](/docs/terminology#propose) or [attesting](/docs/terminology#attest) to blocks on the Agora chain, and receive either rewards or penalties to the initial deposit based upon their overall performance.
...
2. Checks if public key corresponding to the validator instance has been activated on the Agora chain.
...
1. The [canonical head block](/docs/terminology#canonical-head-block) is fetched from the Agora node and its root is used as the parent root of the new block.
...
5. The latest fork data from the Agora chain is retrieved.
6. A randao reveal is generated by [BLS](/docs/how-agora-cl-works/bls-cryptography) signing the epoch.
7. All pending [attestations](/docs/terminology#attest) from the Agora node are fetched.
...
10. The block is [proposed](/docs/terminology#propose) by sending it to the Agora node via [gRPC](./agora-cl-public-api).
...
2. A request is made to the Agora node for the information required to attest a block.
...
5. Halfway through the slot duration, the attestation is sent to the Agora node via [gRPC](./agora-cl-public-api).
