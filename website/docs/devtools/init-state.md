...
**Where the feature lives in Agora-cl:** The feature is fully contained within the following folder/package: [/beacon-chain/sync/initial-sync/](https://github.com/zeroone-boa/agora-cl/tree/develop/beacon-chain/sync/initial-sync)
...
See: [service.go](https://github.com/zeroone-boa/agora-cl/blob/ce397ce797c33dbcf77fa7670c356844ef6aad43/beacon-chain/sync/initial-sync/service.go#L46), [round_robin.go](https://github.com/zeroone-boa/agora-cl/blob/develop/beacon-chain/sync/initial-sync/round_robin.go)
...
- When started, queue creates the internal fetcher service, which is responsible for lower level data fetching functionality i.e. queue manages the overall scheduling process, while blocks fetcher is responsible for talking to peers, requesting and processing their data (see [blocks_queue.go:newBlocksQueue](https://github.com/zeroone-boa/agora-cl/blob/ce397ce797c33dbcf77fa7670c356844ef6aad43/beacon-chain/sync/initial-sync/blocks_queue.go#L100). Block fetcher exposes the output channel which the queue keeps waiting on up until all the necessary blocks are fetched, at that point the queue closes the fetcher's context thus closing that sub-service as well see: [blocks_queue.go:loop](https://github.com/zeroone-boa/agora-cl/blob/ce397ce797c33dbcf77fa7670c356844ef6aad43/beacon-chain/sync/initial-sync/blocks_queue.go#L255).
...
See: [blocks_queue.go](https://github.com/zeroone-boa/agora-cl/blob/ce397ce797c33dbcf77fa7670c356844ef6aad43/beacon-chain/sync/initial-sync/blocks_queue.go#L73), [Queue.onScheduleEvent()](https://github.com/zeroone-boa/agora-cl/blob/ce397ce797c33dbcf77fa7670c356844ef6aad43/beacon-chain/sync/initial-sync/blocks_queue.go#L281), [Queue.onDataReceivedEvent()](https://github.com/zeroone-boa/agora-cl/blob/ce397ce797c33dbcf77fa7670c356844ef6aad43/beacon-chain/sync/initial-sync/blocks_queue.go#L299), [Queue.onReadyToSendEvent()](https://github.com/zeroone-boa/agora-cl/blob/ce397ce797c33dbcf77fa7670c356844ef6aad43/beacon-chain/sync/initial-sync/blocks_queue.go#L334), [Queue.onProcessSkippedEvent()](https://github.com/zeroone-boa/agora-cl/blob/ce397ce797c33dbcf77fa7670c356844ef6aad43/beacon-chain/sync/initial-sync/blocks_queue.go#L383), [Queue.onCheckStaleEvent()](https://github.com/zeroone-boa/agora-cl/blob/ce397ce797c33dbcf77fa7670c356844ef6aad43/beacon-chain/sync/initial-sync/blocks_queue.go#L442)

###   FSM Manager and FSMs

Each FSM [(see fsm.go:stateMachine)](https://github.com/zeroone-boa/agora-cl/blob/ce397ce797c33dbcf77fa7670c356844ef6aad43/beacon-chain/sync/initial-sync/fsm.go#L43)  has the following data associated with it:
...
|---------------|--------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Tick          | New                | [Queue.onScheduleEvent()](https://github.com/zeroone-boa/agora-cl/blob/ce397ce797c33dbcf77fa7670c356844ef6aad43/beacon-chain/sync/initial-sync/blocks_queue.go#L281)       |
| Tick          | Data parsed        | [Queue.onReadyToSendEvent()](https://github.com/zeroone-boa/agora-cl/blob/ce397ce797c33dbcf77fa7670c356844ef6aad43/beacon-chain/sync/initial-sync/blocks_queue.go#L334)    |
| Tick          | Skipped            | [Queue.onProcessSkippedEvent()](https://github.com/zeroone-boa/agora-cl/blob/ce397ce797c33dbcf77fa7670c356844ef6aad43/beacon-chain/sync/initial-sync/blocks_queue.go#L383) |
| Tick          | Sent               | [Queue.onCheckStaleEvent()](https://github.com/zeroone-boa/agora-cl/blob/ce397ce797c33dbcf77fa7670c356844ef6aad43/beacon-chain/sync/initial-sync/blocks_queue.go#L442)     |
| Data Received | Scheduled          | [Queue.onDataReceivedEvent()](https://github.com/zeroone-boa/agora-cl/blob/ce397ce797c33dbcf77fa7670c356844ef6aad43/beacon-chain/sync/initial-sync/blocks_queue.go#L299)   |
...
See: [fsm.go:stateMachineManager](https://github.com/zeroone-boa/agora-cl/blob/ce397ce797c33dbcf77fa7670c356844ef6aad43/beacon-chain/sync/initial-sync/fsm.go#L35)
...
- Whenever a queue has less than a [predefined](https://github.com/zeroone-boa/agora-cl/blob/ce397ce797c33dbcf77fa7670c356844ef6aad43/beacon-chain/sync/initial-sync/blocks_queue.go#L31) number of machines (currently 8), a new machine is added.
...
See: [blocks_queue_utils.go:resetFromFork()](https://github.com/zeroone-boa/agora-cl/blob/ce397ce797c33dbcf77fa7670c356844ef6aad43/beacon-chain/sync/initial-sync/blocks_queue_utils.go#L11) and [blocks_queue_utils.go:resetFromSlot()](https://github.com/zeroone-boa/agora-cl/blob/ce397ce797c33dbcf77fa7670c356844ef6aad43/beacon-chain/sync/initial-sync/blocks_queue_utils.go#L44)

###   Blocks Fetcher

The fetcher component (see [blocks_fetcher.go:blocksFetcher](https://github.com/zeroone-boa/agora-cl/blob/ce397ce797c33dbcf77fa7670c356844ef6aad43/beacon-chain/sync/initial-sync/blocks_queue_utils.go#L44)) is responsible for handling p2p communication, and does the actual data requesting and fetching. It is a very simple component operating in the following way:
...
- Fetcher is smart enough to honour rate limits and while being highly concurrent, never requests at a higher pace than is allowed by Agora-cl beacon nodes (see [getPeerLock()](https://github.com/zeroone-boa/agora-cl/blob/ce397ce797c33dbcf77fa7670c356844ef6aad43/beacon-chain/sync/initial-sync/blocks_fetcher_peers.go#L21)). So, fetching nodes should never receive a bad score from fellow Agora-cl peers (while it is theoretically possible that other clients still consider our fetcher aggressive, in reality Agora-cl has one of the strictest limits, thus in practice this has never happened).
...
See: [blocks_fetcher.go](https://github.com/zeroone-boa/agora-cl/blob/ce397ce797c33dbcf77fa7670c356844ef6aad43/beacon-chain/sync/initial-sync/blocks_fetcher.go#L68), [blocks_fetcher_peers.go](https://github.com/zeroone-boa/agora-cl/blob/ce397ce797c33dbcf77fa7670c356844ef6aad43/beacon-chain/sync/initial-sync/blocks_fetcher_peers.go#L1), [blocks_fetcher_utils.go](https://github.com/zeroone-boa/agora-cl/blob/ce397ce797c33dbcf77fa7670c356844ef6aad43/beacon-chain/sync/initial-sync/blocks_fetcher_utils.go#L1)
...
Our backtracking algorithm is pretty simple (code is abridged, see full version [here](https://github.com/zeroone-boa/agora-cl/blob/ce397ce797c33dbcf77fa7670c356844ef6aad43/beacon-chain/sync/initial-sync/blocks_fetcher_utils.go#L1470):)
...
And the corresponding implementation can be found in [blocks_fetcher_utils.go:nonSkippedSlotAfter](https://github.com/zeroone-boa/agora-cl/blob/ce397ce797c33dbcf77fa7670c356844ef6aad43/beacon-chain/sync/initial-sync/blocks_fetcher_utils.go#L34), here it signature:
...
See: [blocks_queue.go:onProcessSkippedEvent()](https://github.com/zeroone-boa/agora-cl/blob/ce397ce797c33dbcf77fa7670c356844ef6aad43/beacon-chain/sync/initial-sync/blocks_queue.go#L383)
...
We have quite sophisticated scoring mechanisms already in place, and we’re [continuingly](https://github.com/zeroone-boa/agora-cl/issues/6622) building on them. Currently, the peer scorer service is able to track peer’s performance (on how many requests, how many useful blocks -- that’s blocks advancing the head -- have been returned). Peers scoring higher will have a better chance of being selected (see [blocks_fetcher_peers.go](https://github.com/zeroone-boa/agora-cl/blob/ce397ce797c33dbcf77fa7670c356844ef6aad43/beacon-chain/sync/initial-sync/blocks_fetcher_peers.go#L94) for full code):
```go
...
Since initial synchronization interacts heavily with the P2P layer, we rely on mocks to simulate network mesh. The easiest way to start the test is to use [initializeTestServices()](https://github.com/zeroone-boa/agora-cl/blob/ce397ce797c33dbcf77fa7670c356844ef6aad43/beacon-chain/sync/initial-sync/initial_sync_test.go#L74):
...
See, [TestBlocksQueue_Loop()](https://github.com/zeroone-boa/agora-cl/blob/ce397ce797c33dbcf77fa7670c356844ef6aad43/beacon-chain/sync/initial-sync/blocks_queue_test.go#L134) as an example of testing block syncing as a whole.
...
- [TestBlocksQueue_stuckWhenHeadIsSetToOrphanedBlock()](https://github.com/zeroone-boa/agora-cl/blob/ce397ce797c33dbcf77fa7670c356844ef6aad43/beacon-chain/sync/initial-sync/blocks_queue_test.go#L134)
- [TestBlocksQueue_stuckInUnfavourableFork()](https://github.com/zeroone-boa/agora-cl/blob/ce397ce797c33dbcf77fa7670c356844ef6aad43/beacon-chain/sync/initial-sync/blocks_queue_test.go#L1026)
...
To run tests using vanilla Go, just use `go test` (to understand the reasons for passing the `develop` tag see our [DEPENDENCIES](https://github.com/zeroone-boa/agora-cl/blob/develop/DEPENDENCIES.md#running-tests)):
